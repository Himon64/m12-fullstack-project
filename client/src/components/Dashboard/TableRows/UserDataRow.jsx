...
const UserDataRow = ({user}) => {
  ...
  return (
    <tr>
      ...
      <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
        <p className={
          `${status === 'requested' ? 'text-yellow-500' : status === 'verified' ? 'text-green-500' : 'text-red-500'} whitespace-no-wrap`
        } >
      ...
      <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
        ...
        {/* Modal */}
        <UpdateUserRoleModal isOpen={isOpen} setIsOpen={setIsOpen} role={role} userEmail = {email} />
      </td>
    </tr>
  )
}
